...
def softmax_overflow_demo(large_value):
    ...
    softmax = np.exp(large_value)
    overflow = np.isinf(softmax)
    return {'naive_exp': softmax, 'overflowed': overflow}
...
def stable_softmax_1d(logits):
    ...
    c = max_along_axis(logits, 0)
    exp_logits = array_exp(logits - c)
    return exp_logits/np.sum(exp_logits)
...
def encode_corpus_to_int_array(text, stoi):
    ...
    res = encode_string(text, stoi)
    return make_1d_array(res)
...
def slice_y_at_offset(data, i, block_size):
    ...
    return slice_x_at_offset(data, i+1, block_size)
...
def sample_random_batch_offsets(data_len, block_size, batch_size, rng):
    ...
    N = data_len
    T = block_size
    return rng.integers(0, high=N-T, size=batch_size)
...
def stack_x_batch(data, offsets, block_size):
    ...
    res = []
    for i in offsets:
        t = slice_x_at_offset(data, i, block_size)
        res.append(t)
    return np.array(res)
...
def stack_y_batch(data, offsets, block_size):
    ...
    res = []
    for i in offsets:
        t = slice_y_at_offset(data, i, block_size)
        res.append(t)
    return np.array(res)
...
def allocate_count_matrix(vocab_size):
    ...
    matrix = make_2d_zeros(vocab_size, vocab_size)
...
def normalize_counts_to_probs(n_matrix):
    ...
    sum_row = row_sums_of_counts(n_matrix)
    return n_matrix / sum_row
...
def generate_sequence(p_matrix, start_id, length, rng):
    ...
    for t in range(1, length):
        sequence[t] = sample_next_token(p_matrix, sequence[t - 1], rng)
...
def sum_negative_log_probs(p_matrix, data):
    ...
    for i in range(len(data)-1):
        k = log_prob_of_pair(p_matrix, data[i], data[i+1])
        if k < 0:
            res += k
    return -res
...
def one_hot_encode_batch(ids, vocab_size):
    ...
    N = len(ids)
    res = make_2d_zeros(N, vocab_size)
    for i in range(N):
        t = ids[i]
        res[i][t] = 1.0
    return res
